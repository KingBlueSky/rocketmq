...
public class RouteInfoManager {
    ...
    /**
     * broker状态信息，nameServer每次收到心跳包时都会替换该信息
     * key --> broker ip地址 eg. 10.68.10.50:10000
     * value --> brokerName状态信息
     */
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                // step 1:路由注册加写锁，防止并发修改routeInfoManager中的路由表
                this.lock.writeLock().lockInterruptibly();

                // 判断broker所属集群是否存储，如果不存在则创建，然后将brokerName加入到集群集合中
                Set<String> brokerNames = this.clusterAddrTable.computeIfAbsent(clusterName, k -> new HashSet<>());
                brokerNames.add(brokerName);

                boolean registerFirst = false;

                // step 2:维护brokerData信息，先从brokerAddrTable中根据brokerName尝试获取broker信息，如果不存在，则新建brokerData对象
                // 并放入brokerAddrTable中， registerFirst设置为true
                ...
                // 获取知道brokerName下的broker地址集合master，salve等
                Map<Long, String> brokerAddrsMap = brokerData.getBrokerAddrs();

                //Switch slave to master: first remove <1, IP:PORT> in namesrv, then add <0, IP:PORT>
                //The same IP:PORT must only have one record in brokerAddrTable
                // 移除已经存在的【brokerId不同，brokerAddr相同】的broker信息
                brokerAddrsMap.entrySet().removeIf(item -> null != brokerAddr && brokerAddr.equals(item.getValue()) && brokerId != item.getKey());

                // 如果brokerData存在，则直接替换原先的，registerFirst设置为false，表示非第一次注册
                String oldAddr = brokerData.getBrokerAddrs().put(brokerId, brokerAddr);
                registerFirst = registerFirst || (null == oldAddr);

                // topicConfigWrapper不为空，且broker为master
                if (null != topicConfigWrapper && MixAll.MASTER_ID == brokerId) {
                    // broker topic配置信息发生变化 || 初次注册。则需要创建或者更新topic路由元数据，填充topicQueueTable
                    // 其实就是为默认主题自动注册路由信息，其中包含MixAll.DEFAULT_TOPIC的路由信息。当生产者发生消息到topic时
                    // 如果该主题未创建并且brokerConfig的autoCreateTopicEnable为true，将返回MixAll.DEFAULT_TOPIC的路由信息
                ...
                // 更新brokerLiveInfo，brokerLiveInfo是执行路由删除的重要依据
                ...
                // 如果次broker是从节点，则需要查找该broker的master节点信息，并更新对应的masterAddr属性
    ...
    private void createAndUpdateQueueData(final String brokerName, final TopicConfig topicConfig) {
        ...
        // TODO: 2021/5/25 为啥要创建读写队列，是什么考虑
    ...
    /**
     * scanNotActiveBroker在nameServer中每10秒执行一次
     * 遍历brokerLiveInfo路由表，检测brokerLiveInfo的lastUpdateTimestamp上次收到心跳包的时间如果超过120s
     * nameSever则认为该broker已不可用，故需要将它移除，关闭channel，然后删除与该broker相关的路由信息
     * 路由表维护过程，需要申请写锁在onChannelDestroy方法中执行
     *
     * // TODO: 2021/5/25 已经在这里从 brokerLiveTable中移除了，为啥还要在onChannelDestroy中进行移除
     */
    ...
    public void onChannelDestroy(String remoteAddr, Channel channel) {
        ...
        if (brokerAddrFound != null && brokerAddrFound.length() > 0) {
            ...
            try {
                try {
                    // 申请写锁，从brokerLiveTable和filterServerTable中移除
                    this.lock.writeLock().lockInterruptibly();
                    this.brokerLiveTable.remove(brokerAddrFound);
                    this.filterServerTable.remove(brokerAddrFound);

                    // 维护brokerAddrTable，遍历brokerAddrTable，从brokerData的brokerAddrs中找到具体的broker
                    // 从brokerData中移除，如果移除后在brokerData中不在包含其他broker，则在brokerAddrTable中移除该brokerName对应的条目
                    ...
                    // 根据brokerName，从clusterAddrTable中找到Broker并从集群中删除
                    ...
                    // 如果只包含待移除broker的队列的话，从路由表中删除该topic
                    if (removeBrokerName) {
